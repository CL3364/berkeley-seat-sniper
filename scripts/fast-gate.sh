...
# Keep only JS/TS source files.
FILES=$(printf '%s\n' "$CHANGED" | grep -E '\.(ts|tsx|js|jsx|mjs|cjs)$' || true)

# No code changed (e.g. a spec or review task) → nothing to gate.
[ -z "$FILES" ] && { echo "fast-gate: no code changes to check." >&2; exit 0; }
...
if command -v npx >/dev/null 2>&1 && [ -f node_modules/.bin/prettier ]; then
  printf '%s\n' "$FILES" | xargs npx prettier --check >/dev/null 2>&1 || fail "formatting (prettier) on changed files"
fi
...
if command -v npx >/dev/null 2>&1 && [ -f node_modules/.bin/eslint ]; then
  printf '%s\n' "$FILES" | xargs npx eslint --max-warnings=0 >/dev/null 2>&1 || fail "lint (eslint) on changed files"
fi
